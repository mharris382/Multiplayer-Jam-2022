...
export var max_jump_height : int = 150 setget set_max_jump_height
# The minimum jump height (tapping jump)
export var min_jump_height : int = 40 setget set_min_jump_height
# The height of your jump in the air
export var double_jump_height : int = 100 setget set_double_jump_height
# How long it takes to get to the peak of the jump in seconds
export var jump_duration : float = 0.3 setget set_jump_duration
# Multiplies the gravity by this while falling
export var falling_gravity_multiplier : float = 1.5
# Set to 2 for double jump
export var max_jump_amount : int = 1
export var max_acceleration : float = 4000
...
func _physics_process(delta:float):
	...
	if Input.is_action_just_pressed(input_jump):
		if Input.is_action_pressed("move_down"):
			var pos = position
			position = Vector2(pos.x, pos.y + 1)
		else:
			holding_jump = true
...
func calculate_gravity(p_max_jump_height:int, p_jump_duration:float):
...
func calculate_jump_velocity(p_max_jump_height:int, p_jump_duration:float):
...
func calculate_jump_velocity2(p_max_jump_height:int, p_gravity:float):
	...
	return sqrt(-2 * p_gravity * float(p_max_jump_height))


func calculate_release_gravity_multiplier(p_jump_velocity : float, p_min_jump_height : int, p_gravity:float):
	...
	var release_gravity = 0 - pow(p_jump_velocity, 2) / (2 * float(p_min_jump_height))
...
func set_max_jump_height(value : int):
...
func set_jump_duration(value :float):
	jump_duration = value
...
func set_min_jump_height(value : int):
	min_jump_height = value
	release_gravity_multiplier = calculate_release_gravity_multiplier(
		jump_velocity, min_jump_height, default_gravity)


func set_double_jump_height(value : int):
